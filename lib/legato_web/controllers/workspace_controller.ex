defmodule LegatoWeb.WorkspaceController do
  ...
  alias Legato.Chat.Schemas.{Workspace}

  def get_workspace(conn, %{"workspace_id" => workspace_id}) when is_binary(workspace_id) do
  ...
  def get_workspace_users(conn, %{"workspace_id" => workspace_id}) when is_binary(workspace_id) do
    with  true                              <- get_session(conn, :signed_in),
          ^workspace_id                     <- get_session(conn, :workspace_id),
